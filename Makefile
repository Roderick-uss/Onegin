...
S_PREFIX = sources/
T_PREFIX = text_files/
SOURCES_PREPROC = loger preprocessor commoner
SOURCES_MAIN = main commoner onegin_text string_comparators sorter loger
OBJECTS_PREPROC := $(patsubst %,$(O_PREFIX)%.o,$(SOURCES_PREPROC))
OBJECTS_MAIN := $(patsubst %,$(O_PREFIX)%.o,$(SOURCES_MAIN))
HEADER_LIST =$(S_PREFIX)*.h

all: main preprocessor

run: main
	@./$(B_PREFIX)main

run_all: main preproc
	@./$(B_PREFIX)preproc
	@./$(B_PREFIX)main

preproc: $(OBJECTS_PREPROC)
	@mkdir -p $(B_PREFIX)
